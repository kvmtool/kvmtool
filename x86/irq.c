...
/*
 * set irq level. If an edge is detected, the the IRR is set to 1
 */
...
static void pic_update_irq(struct kvm_pic *vpic)
{
	int irq, irq2;
	...
	pic_irq_request(vpic->kvm, irq >= 0);
}
...
int kvm_pic_set_irq(struct kvm_pic *vpic, int irq, int level)
{
	...
	BUG_ON(irq < 0 || irq >= KVM_PIC_NUM_PINS);
...
static void kvm_pic_reset(struct kvm_kpic_state *s)
{
	...
	for (irq = 0; irq < KVM_PIC_NUM_PINS / 2; irq++)
		if (edge_irr & (1 << irq))
			pic_clear_isr(s, irq);
}
